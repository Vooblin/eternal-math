...
class EternalMathCLI:
    ...
    def _plot_function(self, args: List[str]) -> None:
        ...
        try:
            ...
            success = self.visualizer.plot_function(
                expression, x_range=x_range, title=f"Graph of f(x) = {expression}"
            )
    ...
    def _plot_sequence(self, args: List[str]) -> None:
        ...
        try:
            ...
            if seq_type == "fibonacci":
                ...
                sequence = primes[
                    : min(20, len(primes))
                ]  # Limit to first 20 for visibility
            ...
            success = self.visualizer.plot_sequence(
                [float(x) for x in sequence], title=title
            )
    ...
    def _plot_collatz(self, args: List[str]) -> None:
        ...
        try:
            ...
            print(
                "Error: Invalid number format. Use comma-separated positive integers\n"
            )
        except Exception as e:
    ...
    def _plot_comparative(self, args: List[str]) -> None:
        ...
        try:
            ...
            for seq_type in [type1, type2]:
                if seq_type == "fibonacci":
                    sequences["Fibonacci"] = [
                        float(x) for x in fibonacci_sequence(min(n, 15))
                    ]
                elif seq_type == "primes":
                    ...
                    sequences["Euler φ(n)"] = [
                        float(euler_totient(i)) for i in range(1, min(n, 15) + 1)
                    ]
                elif seq_type == "collatz":
                    # Use length of Collatz sequences
                    sequences["Collatz Steps"] = [
                        float(len(collatz_sequence(i)))
            ...
            success = self.visualizer.plot_comparative_sequences(
                sequences, title=f"Comparison: {type1.title()} vs {type2.title()}"
            )
        ...
        if args and args[0] == "full":
            ...
            try:
                ...
                benchmark.time_function(
                    lambda: [gcd(i, 100) for i in range(1, 51)], iterations=5
                )
